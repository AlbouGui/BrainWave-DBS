...
%Place triggers encoded in a excel table into raw LFP data
...
colors = {'b', 'c', 'g', 'y'}; % Blue, Red, Green, Magenta
...
% Store subplot handles for linking
subplot_handles = zeros(4, 1);

for cond = 1:4
    subplot_handles(cond) = subplot(4, 1, cond);
    ...
    if d.raw{cond}.initialized && ~isempty(d.raw{cond}.LFP)
        ...
        % Set tick direction to outside for empty plots too
        set(gca, 'TickDir', 'out');
...
% Link both x and y axes across all subplots
...
fprintf('X and Y axes are now linked - zooming/panning will sync across all subplots.\n');
fprintf('Tick marks are positioned outside the axes for cleaner appearance.\n');
