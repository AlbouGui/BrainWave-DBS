...
for k = 1:4
    if ~isempty(segmented_data.(['condition_' num2str(k)]))
        ...
        for ep = 1:num_epochs
            ...
            %% STFT Analysis
            % Perform STFT
            % Use the same window whose energy we measured 
            [S, F, T] = spectrogram(epoch_data, stft_win, overlap, nfft, Fs); 

            % One-sided doubling vector (don't double DC/Nyquist)
            K = 2*ones(size(F));
            K(1) = 1;
            if rem(nfft,2)==0 && abs(F(end) - Fs/2) < 1e-9
                K(end) = 1;
            end

            % PSD (V^2/Hz), Welch normalization
            psd = (K .* mean(abs(S).^2, 2)) / (Fs * win_energy);

            % Store PSD spectrogram (V^2/Hz)
            if ep == 1
                all_freqs = F;
            ...
            all_psds(:, ep) = psd;
            stft_spectrograms(:, :, ep) = (K .* abs(S).^2) / (Fs * win_energy);
            ...
            %% Calculate band powers (unit V^2) for all 3 methods
        ...
        mean_spectrogram_hilbert = bsxfun(@rdivide, mean(all_envelopes,3), enbw_vec); 
...
%% Plot1: Combined PSDs from all three techniques per condition
...
% User selection for condition (with default to first active condition)
selected_condition_hilb = active_conditions(1); % Default 
user_input_hilb = input(sprintf('Enter condition number to display individual trial spectrogram - Hilbert (default: %d): ', selected_condition_hilb), 's');
if ~isempty(user_input_hilb)
    tmp = str2double(user_input_hilb);
    if ismember(tmp, active_conditions)
        selected_condition_hilb = tmp;
    else
        fprintf('Invalid condition selected. Using default condition %d.\n', selected_condition_hilb);
    end
end

% Get the results for selected condition
condition_results = hilbert_results.(['condition_' num2str(selected_condition_hilb)]);
...
for trial = 1:num_trials
    ...
    % PSD (V^2/Hz)
    trial_spectrogram = bsxfun(@rdivide, condition_results.envelopes(:, :, trial), condition_results.enbw_hz);
    imagesc(condition_results.time_vec, condition_results.frequencies, 10*log10(trial_spectrogram));
...
% Calculate STFT-aligned spectrograms PSD (V²/Hz) for all trials to find global maximum
all_trials_spectrograms = zeros(size(condition_results.envelopes));
for trial = 1:num_trials
    all_trials_spectrograms(:, :, trial) = bsxfun(@rdivide, condition_results.envelopes(:, :, trial), condition_results.enbw_hz);
...
% Add a main title for the entire figure
sgtitle(['Condition ' num2str(selected_condition_hilb) ' - All Individual Trial Hilbert Spectrograms - RELATIVE POWER (N=' num2str(num_trials) ')']);%% End
